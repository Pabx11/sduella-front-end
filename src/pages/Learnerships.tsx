...
const API_JOB_TYPES = new Set<OpportunityType>([
  "job",
  "internship",
  "learnership",
  "apprenticeship",
...
const HOW_IT_WORKS = [
  {
    ...
    desc: "Search current learnerships and confirm that you meet the provider requirements.",
  },
  {
    step: "02",
    title: "Apply with the provider",
    desc: "Use the official application link and submit the requested documents.",
  },
  {
    step: "03",
    title: "Sign an agreement",
    desc: "Successful applicants enter an agreement with an employer and training provider.",
  },
  {
    step: "04",
    title: "Learn and work",
    desc: "Complete structured learning and supervised workplace experience while receiving a stipend.",
  },
  {
    step: "05",
    title: "Earn the qualification",
    desc: "Complete the assessments to earn the registered occupational or NQF-aligned qualification.",
  },
];
...
const SOURCE_LABELS: Record<Opportunity["source"], string> = {
  ...
  eu_funding: "EU Funding & Tenders",
  uct_funding: "University of Cape Town",
...
const formatType = (type: OpportunityType) =>
...
export default function Learnerships({
  ...
  const activeType: JobType = JOB_TYPE_VALUES.has(requestedType as JobType)
  ...
  useEffect(() => {
    ...
    searchOpportunities({
      group: "jobs",
      query: [activeType === "all" ? "" : activeType, submittedSearch]
        .filter(Boolean)
        .join(" "),
      location,
      country,
      remote:
        workArrangements.length === 1
          ? workArrangements[0] === "remote"
          : undefined,
      ...
      .then((nextResult) => {
        setResult((currentResult) => {
          ...
          return {
            ...
            withheld_unverified:
              currentResult.withheld_unverified +
              nextResult.withheld_unverified,
            withheld_expired:
              currentResult.withheld_expired + nextResult.withheld_expired,
            withheld_unreachable:
              currentResult.withheld_unreachable +
              nextResult.withheld_unreachable,
          };
        });
      })
      .catch((fetchError) => {
  ...
  const opportunities = useMemo(() => {
    const items =
      result?.items.filter((item) =>
        API_JOB_TYPES.has(item.opportunity_type),
      ) ?? [];
    const filtered =
      activeType === "all"
        ? items
        : items.filter((item) => item.opportunity_type === activeType);
    ...
    if (sortBy === "newest") {
      ...
      sorted.sort(
        (a, b) =>
          new Date(a.closing_date || "9999-12-31").getTime() -
          new Date(b.closing_date || "9999-12-31").getTime(),
      );
    } else if (sortBy === "pay") {
      sorted.sort(
        (a, b) =>
          Number(Boolean(b.amount_or_salary)) -
          Number(Boolean(a.amount_or_salary)),
      );
    }
    return sorted;
  ...
  return (
    <div className={embedded ? "" : "pt-[62px]"}>
      ...
      {!embedded && (
        <section className="bg-black text-white py-24 px-6 md:px-12 relative overflow-hidden">
          <div className="max-w-7xl mx-auto relative z-10">
            <span className="font-syne text-[11px] font-bold tracking-widest uppercase text-green block mb-4">
              {isLearnershipView ? "Bridge to Work" : "Live Opportunities"}
            </span>
            <h1 className="text-4xl sm:text-5xl md:text-7xl font-extrabold tracking-tighter mb-6 leading-[0.9]">
              {isLearnershipView ? "Learnerships" : "Jobs and early careers"}
              <br />
              <span className="text-green">
                {isLearnershipView ? "that pay." : "from live sources."}
              </span>
            </h1>
      ...
      {isLearnershipView && (
        <section className="py-12 sm:py-16 lg:py-20 px-4 sm:px-6 md:px-12 bg-off-white border-b border-grey-200">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
              {HOW_IT_WORKS.map((s, i) => (
                ...
                >
                  <div className="text-3xl font-extrabold text-blue/20 font-syne mb-3">
                    {s.step}
                  </div>
            ...
            <div className="mt-10 p-6 bg-green/5 border border-green/20 rounded-sm flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              <p className="text-sm text-grey-700 leading-relaxed">
                <strong>Learnership vs Internship:</strong> A learnership is
                SETA-registered and results in a nationally recognised NQF
                qualification. An internship is a company-specific work
                placement with no formal qualification. Learnerships have
                stronger legal protections and a clearer qualification outcome.
              </p>
            </div>
          </div>
        </section>
      ...
      <section className="py-12 sm:py-16 lg:py-20 px-4 sm:px-6 md:px-12">
        <div
          className="max-w-5xl min-[1650px]:max-w-7xl mx-auto relative"
          aria-busy={loading}
        >
          <AdSenseWideRails />
          <div className="mb-8 sm:mb-12">
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight mb-2">
              {isLearnershipView ? "Open Learnerships" : "Open Opportunities"}
            </h2>
          ...
          <div className="space-y-4 mb-10">
            <div className="flex gap-2 overflow-x-auto pb-1 sm:flex-wrap">
              {JOB_TYPES.map((type) => (
                <button
                  ...
                  className={cn(
                    "shrink-0 px-4 py-2.5 rounded-sm border text-xs font-bold transition-colors",
                    activeType === type.value
                      ? "bg-black border-black text-white"
                      : "bg-white border-grey-200 text-grey-600 hover:border-blue hover:text-blue",
                  )}
                >
                  {type.label}
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3 items-end">
              <div>
                <label
                  ...
                  className="block text-xs font-bold tracking-wider uppercase text-grey-500 mb-2"
                >
                  Profession, skill or employer
                </label>
                <input
                  ...
                  placeholder="Type any profession, skill or employer…"
                  autoComplete="off"
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-4 border border-grey-200 rounded-sm bg-white text-sm font-semibold focus:outline-none focus:border-blue transition-colors appearance-none cursor-pointer"
                >
              ...
              <div>
                ...
                <select
                  ...
                  value={duration}
                  onChange={(event) => {
                    setDuration(event.target.value as JobDuration);
                    setPage(1);
                  }}
                  className="w-full px-4 py-4 border border-grey-200 rounded-sm bg-white text-sm font-semibold focus:outline-none focus:border-blue appearance-none cursor-pointer"
                >
              ...
              <div>
                <div className="block text-xs font-bold tracking-wider uppercase text-grey-500 mb-2">
                  Work arrangement{" "}
                ...
                <div
                  ...
                  role="group"
                  aria-label="Work arrangement; choose either, both or neither"
                >
                  {(
                    [
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "min-h-12 px-3 py-3 border rounded-sm text-xs font-bold transition-colors",
                          selected
            ...
            {(search ||
              ...
              <button
                ...
                className="text-xs font-bold text-blue hover:underline self-start"
              >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 pt-2 border-t border-grey-100">
              <span className="shrink-0 text-xs font-bold tracking-wider uppercase text-grey-500">
                Sort by
              </span>
              <div
                className="flex gap-2 overflow-x-auto pb-1"
                role="group"
                aria-label="Sort loaded job results"
              >
                {JOB_SORTS.map((option) => (
                  <button
                    ...
                    className={cn(
                      "shrink-0 px-3.5 py-2.5 border rounded-sm text-xs font-bold transition-colors",
                      sortBy === option.value
                        ? "bg-black border-black text-white"
                        : "bg-white border-grey-200 text-grey-600 hover:border-blue hover:text-blue",
                    )}
          ...
          {(result?.errors.length ?? 0) > 0 && !error && (
            <div className="mb-8 border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 rounded-sm">
              Some providers did not respond, so these results may be
              incomplete:{" "}
              {result?.errors
          ...
          {(result?.withheld_unverified ?? 0) > 0 && !error && (
            <div className="mb-8 border border-grey-200 bg-grey-50 p-4 text-sm text-grey-600 rounded-sm">
              {result?.withheld_unverified} listing
              {result?.withheld_unverified === 1 ? " was" : "s were"} excluded
              because no safe official application or instruction page could be
              verified.
            </div>
          )}
          {(result?.withheld_unreachable ?? 0) > 0 && !error && (
            <div className="mb-6 border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
              {result?.withheld_unreachable} official source page
              {result?.withheld_unreachable === 1 ? " was" : "s were"}{" "}
              temporarily unreachable and excluded. Refresh to retry.
            </div>
          )}

          {loading && result && !error && (
            <div
              className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-blue"
              role="status"
              aria-live="polite"
            >
          ...
          {error ? (
            <div className="text-center py-20 border border-dashed border-red/30 bg-red/5 rounded-sm">
              ...
              <p className="font-bold mb-2">Unable to load opportunities</p>
              <p className="text-sm text-grey-600 max-w-xl mx-auto mb-6">
                {error}
              </p>
              <button
                type="button"
                onClick={() => setRetryKey((value) => value + 1)}
                className="inline-flex items-center gap-2 px-5 py-3 bg-black text-white rounded-sm text-xs font-bold uppercase tracking-wider"
              >
            ...
            <>
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                        <div className="min-w-0">
                          {/* <span className="text-[11px] font-bold tracking-wider uppercase px-2.5 py-1.5 rounded-sm inline-block mb-2 bg-green/10 text-green">
                            ● Current
                          </span> */}
                          <div className="font-syne text-xs font-bold tracking-wider uppercase text-blue leading-relaxed [overflow-wrap:anywhere]">
                            {opportunity.organisation}
                          </div>
                        </div>
                        <span className="self-start shrink-0 text-[11px] font-bold uppercase tracking-wider text-grey-500 bg-grey-50 px-2.5 py-1.5 rounded-sm">
                          {formatType(opportunity.opportunity_type)}
                        </span>
                      </div>

                      <h3 className="font-extrabold text-lg mb-2 leading-snug break-words">
                        {opportunity.title}
                      </h3>
                      <p className="text-sm text-grey-600 leading-relaxed flex-1 mb-5 line-clamp-4">
                        {opportunity.description ||
                          "Open the provider listing for the full description."}
                      </p>

                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-4 border-t border-grey-50">
                        <div>
                          <div className="text-[11px] font-bold uppercase tracking-wider text-grey-500 mb-1">
                            Salary / stipend
                          </div>
                          <div className="text-sm font-bold text-green [overflow-wrap:anywhere]">
                        ...
                        <div className="sm:col-span-2 flex items-start gap-1.5 text-sm font-semibold text-grey-600 min-w-0">
                          <MapPin
                            size={15}
                          ...
                          <span className="[overflow-wrap:anywhere]">
                            {opportunity.remote
                              ? `Remote · ${opportunity.country_name || "Worldwide"}`
                      ...
                      {/* <div className="mt-4 text-[11px] leading-relaxed uppercase tracking-wider font-bold text-grey-500">
                        {formatRegion(opportunity.region)} ·{" "}
                        {opportunity.country_name || "Worldwide"} ·{" "}
              ...
              <div className="mt-10 flex flex-col items-center gap-3">
                {result?.has_more && page < 100 && (
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        Load more jobs <ArrowRight size={14} />
                      </>
                    )}
                  </button>
                )}
                ...
                <p className="text-xs text-grey-500 text-center">
                  Listings are loaded in provider batches. Use country, location
                  and keyword filters for more relevant results.
                </p>
              </div>
            </>
          ) : (
            <div className="text-center py-20 border border-dashed border-grey-300 rounded-sm">
              <p className="text-grey-500 font-syne italic">
                No available opportunities matched these filters.
              </p>
              <button
                type="button"
                onClick={() => {
                ...
                className="text-[11px] font-bold text-blue hover:underline mt-4"
              >
                Clear filters
              </button>
            </div>
      ...
      {selected && (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-6">
          ...
          >
            <button
              onClick={() => setSelected(null)}
              className="absolute top-3 right-3 sm:top-5 sm:right-5 p-2 bg-white/90 hover:bg-grey-100 rounded-sm transition-colors z-10"
            >
              <X size={18} />
            </button>
            <div className="p-5 pt-14 sm:p-8 lg:p-10">
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                <span className="text-[9px] font-bold tracking-widest uppercase px-2 py-1 rounded-sm bg-green/10 text-green">
                  ● Current
                </span>
                <span className="text-[10px] font-bold tracking-widest uppercase text-blue">
                  {SOURCE_LABELS[selected.source]}
                </span>
                <span className="text-[10px] font-bold tracking-widest uppercase text-grey-400">
                  {formatType(selected.opportunity_type)}
                </span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-extrabold mb-1 [overflow-wrap:anywhere]">
                {selected.title}
              </h2>
              <div className="font-syne font-bold text-grey-500 mb-6">
                ...
                {[
                  {
                    ...
                    value: formatCompensation(selected),
                    highlight: true,
                  },
                  {
                    label: "Location",
                    value: selected.remote
                      ? `Remote · ${selected.country_name || "Worldwide"}`
                      : selected.location ||
                  ...
                  {
                    label: "Deadline",
                    value: formatDate(selected.closing_date),
                  },
                ].map(({ label, value, highlight }) => (
                  <div key={label}>
                    <div className="text-[10px] font-bold tracking-widest uppercase text-grey-400 mb-1">
                      {label}
                    </div>
                    <div
                      className={cn(
              ...
              <div className="mb-8">
                <div className="text-[10px] font-bold tracking-widest uppercase text-grey-400 mb-3">
                ...
                <div className="mb-8">
                  ...
                  <ul className="space-y-2">
                    {selected.requirements.map((requirement) => (
                      <li
                        key={requirement}
                        className="flex items-center gap-3 text-sm text-grey-700"
                      >
              ...
              <p className="text-[10px] text-grey-400 text-center mt-4">
                Applications are completed on the provider's official website.
              </p>
            </div>
          </motion.div>
        </div>
            ...
            <div className="max-w-7xl mx-auto">
              <div className="mb-12">
                ...
                <h2 className="text-3xl font-extrabold tracking-tight mb-2">
                  All 21 SETAs in South Africa
                </h2>
                <p className="text-grey-600 text-sm">
                  Each SETA funds learnerships in its sector. Visit their
                  websites to find additional opportunities not listed above.
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {[
                  ...
                  {
                    name: "CATHSSETA",
                    sector: "Tourism, Arts, Hospitality & Sport",
                    url: "https://www.cathsseta.org.za",
                  },
                  {
                    name: "CETA",
                  ...
                  },
                  {
                    name: "EWSETA",
                    sector: "Energy & Water",
                    url: "https://www.ewseta.org.za",
                  },
                  {
                    name: "FASSET",
                    sector: "Finance & Auditing",
                    url: "https://www.fasset.org.za",
                  },
                  {
                    name: "FoodBev SETA",
                    sector: "Food & Beverage Manufacturing",
                    url: "https://www.foodbev.co.za",
                  },
                  {
                    name: "FP&M SETA",
                    sector: "Clothing, Textiles & Printing",
                  ...
                  },
                  {
                    name: "INSETA",
                    sector: "Insurance",
                    url: "https://www.inseta.org.za",
                  },
                  {
                    name: "LGSETA",
                    sector: "Local Government",
                    url: "https://www.lgseta.org.za",
                  },
                  {
                    name: "merSETA",
                  ...
                  {
                    name: "MICT SETA",
                    sector: "IT, Media & Communications",
                  ...
                  },
                  {
                    ...
                    url: "https://www.pseta.org.za",
                  },
                  {
                    name: "SASSETA",
                    sector: "Safety & Security",
                    url: "https://www.sasseta.org.za",
                  },
                  {
                    name: "Services SETA",
                    sector: "Marketing, HR & Cleaning",
                  ...
                  >
                    <div>
                      <div className="font-bold text-sm group-hover:text-blue transition-colors">
                        {seta.name}
                      </div>
                    ...
                    <ArrowRight
                      size={14}
                      className="text-grey-300 group-hover:text-blue transition-colors flex-shrink-0"
                    />
                  </a>
                ))}
              </div>
          ...
          <section className="py-12 sm:py-16 lg:py-20 px-4 sm:px-6 md:px-12 bg-black text-white">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <p className="text-white/60 mb-8 leading-relaxed">
                A learnership is the fastest route from study to work in South
                Africa. Find your fit, apply early, and earn while you learn.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <a
                  href="/funding-guide#learnership"
                  className="px-8 py-4 border border-white/20 text-white font-syne font-bold text-sm rounded-sm hover:bg-white/5 transition-all"
                >
                  How Learnerships Work
                </a>
                <a
                  href="/bursaries"
                  className="px-8 py-4 bg-white text-black font-syne font-bold text-sm rounded-sm hover:bg-off-white transition-all"
                >
