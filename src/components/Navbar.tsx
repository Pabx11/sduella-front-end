...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: 'Growth & Enterprise', path: '/business-funding?type=business_funding' },
    ],
  },
  ...
  return (
    <nav className="fixed inset-x-0 top-0 h-[62px] bg-white border-b border-grey-200 z-[300] flex items-center justify-between px-6 md:px-12">
      <div className="flex items-center gap-8">
        <a href="/" className="flex items-center cursor-pointer" aria-label="Sduella home">
            ...
            >
              ...
              <AnimatePresence>
                {item.children && openDropdown === item.label && (
                  ...
                  >
                    <div role="menu" className="bg-white border border-grey-200 p-2 shadow-[0_18px_45px_rgba(17,17,17,0.14)]">
      ...
      <div className="hidden lg:flex items-center gap-3">
        {user ? (
          <>
            <a href="/dashboard" className="font-syne text-xs font-bold text-grey-600 hover:text-black">{user.name.split(' ')[0]}</a>
            <button type="button" onClick={onLogout} className="px-4 py-2 border border-grey-200 font-syne text-xs font-bold hover:border-black">Log out</button>
          </>
        ) : (
          <button type="button" onClick={onOpenAuth} className="px-5 py-2.5 bg-black text-white font-syne text-xs font-bold">Sign in / Sign up</button>
        )}
      </div>
        ...
        {isMenuOpen && (
          ...
          >
            <div className="flex flex-col">
              {navItems.map((item) => (
                <div key={item.label} className="border-b border-grey-100 last:border-0">
                  {item.children ? (
                    <>
                      <button
                        type="button"
                    ...
                    <a href={item.path} onClick={closeMenus} className="block py-4 font-syne text-lg font-bold text-black">
                      {item.label}
                    </a>
                  )}
                </div>
              ...
              <div className="pt-4 mt-3 border-t border-grey-200">
                {user ? (
                  <div className="grid grid-cols-2 gap-3">
                    <a href="/dashboard" onClick={closeMenus} className="py-3 text-center bg-black text-white font-syne text-sm font-bold">My account</a>
                    <button type="button" onClick={() => { onLogout(); closeMenus(); }} className="py-3 border border-grey-200 font-syne text-sm font-bold">Log out</button>
                  </div>
                ) : (
                  <button type="button" onClick={() => { onOpenAuth(); closeMenus(); }} className="w-full py-3 bg-black text-white font-syne text-sm font-bold">Sign in or register</button>
                )}
              </div>
            </div>
          </motion.div>
